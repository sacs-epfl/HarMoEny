...
directory_path = "../data/systems/datasets"
...
def create_moe_layer_latency_prob_num_tokens_policy_vs_dataset():
    for moe_layer_idx in range(num_moe_layers):
        ...
        save_pd(pd.DataFrame(df), f"../data_processed/systems/datasets/layer_latency/layer-{moe_layer_idx}_avg_latency_prob_number_tokens_policy_vs_dataset.csv")
...
def create_imbalance_across_iter_and_average():
    for moe_layer_idx in range(num_moe_layers):
        ...
        for dataset in datasets:
            ...
            save_pd(dff, f"../data_processed/systems/datasets/imbalance/layer-{moe_layer_idx}_{dataset}_policy_imbalance_over_iters.csv")
        save_pd(pd.DataFrame(df), f"../data_processed/systems/datasets/imbalance/layer-{moe_layer_idx}_avg_imbalance_policy_vs_dataset.csv")
...
def create_metric_across_iter_and_average():
    ...
    for dataset in datasets:
        ...
        save_pd(dff, f"../data_processed/systems/datasets/metrics/{dataset}_policy_metrics_over_iters.csv")
    save_pd(pd.DataFrame(df), f"../data_processed/systems/datasets/metrics/avg_metrics_policy_vs_dataset.csv")
