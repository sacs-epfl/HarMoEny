...
def plot_workload_duration_proportion_policy_vs_dataset():
    df = pd.read_csv(f"../data_processed/systems/datasets/workload_duration_proportion_policy_vs_dataset.csv", index_col=0)
    ...
    for i, (dataset, positions) in enumerate(zip(pivot_df.index, x_positions)):
        for j, sys in enumerate(pivot_df.columns):
            value = pivot_df.loc[dataset, sys]
            # Plot the bar
            ax.bar(positions[j], value, width=bar_width, color=system_colors[sys], label=sys if i == 0 else "")
            # Add the value above the bar
            ax.text(
                positions[j], value + 0.02, f"{value:.2f}",  # Adjust y-position to be slightly above the bar
    ...
    ax.set_xticklabels(pivot_df.index)
    ax.set_xlabel("Dataset")
    ax.set_ylabel("Speedup over DeepSpeed (x)")
    ax.legend(title="System")

    save_plot(plt, "../figures/systems/datasets/workload_duration_proportion_policy_vs_dataset.png")
...
if __name__ == "__main__":
    ...
    plot_workload_duration_proportion_policy_vs_dataset()
