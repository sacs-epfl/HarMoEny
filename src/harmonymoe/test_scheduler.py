...
class TestScheduler(unittest.TestCase):
    def test_basic(self):
        ...
        meta = [
            ...
            [torch.tensor(0), torch.tensor(20)],
        ]
        topo = [[1], [0]]
    ...
    def test_all_tokens_scheduled(self):
        ...
        scheduler = Scheduler(
            num_experts=num_experts, num_gpus=num_gpus, scheduling_policy="adfabricus"
        )
        ...
        for _ in range(10):
            random.seed(42)
            meta = [
                [torch.tensor(random.randint(0, 1000)) for _ in range(num_experts)]
                for _ in range(num_gpus)
            ]
            schedule = scheduler(meta, topo)
            num_tot_tokens = sum(
                list(map(lambda arr: sum(list(map(lambda x: x.item(), arr))), meta))
            )
            num_scheduled_tokens = sum(
                list(
                    map(
                        lambda experts: sum(list(map(lambda t: sum(t), experts))),
                        schedule,
                    )
                )
            )
