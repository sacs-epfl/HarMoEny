...
def calculate_cost(message, solution, intra_gpus=0):
    ...
    for i in range(len(message)-1):
        if intra_gpus == 0:
            ...
            print(f"THIS IS CURRENT i: {i}, with mes len: {len(message)}, and sol len: {len(solution)}, and i+1 msg: {message[i+1]}")
            count += 1
...
def read_parition(total_number_gpu, use_bipart=False):
    ...
    file_name = f'solutions/solution_exp{num_expert_per_layer}_intra{intra_gpus}_inter{nodes}_cost{avg_cost}_cintra{avg_intra}_cinter{avg_inter}.csv'
