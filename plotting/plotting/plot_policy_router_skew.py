...
def plot_workload_duration_policy_vs_dataset():
    df = pd.read_csv(f"../data_processed/policies/router_skew/workload_duration_policy_vs_dataset.csv", index_col=0)

    df["policy"] = pd.Categorical(df["policy"], categories=policies, ordered=True)

    # Pivot the DataFrame
    pivot_df = df.pivot(index="skew", columns="policy", values="duration (s)")
    ...
    for i, (skew, positions) in enumerate(zip(pivot_df.index, x_positions)):
        for j, sys in enumerate(pivot_df.columns):
            ax.bar(positions[j], pivot_df.loc[skew, sys], width=bar_width, color=policy_colors[sys], label=sys if i == 0 else "")
    ...
    save_plot(plt, "../figures/policies/router_skew/workload_duration_policy_vs_dataset.png")
